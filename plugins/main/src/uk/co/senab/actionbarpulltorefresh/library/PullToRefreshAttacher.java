...
public class PullToRefreshAttacher {
    ...
    final boolean onInterceptTouchEvent(MotionEvent event) {
        if (DEBUG) {
            Log.d("hyytest", "onInterceptTouchEvent: " + event.toString());
        }
       // Log.d("hyytest", "onInterceptTouchEvent: " + event.toString());
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE: {
            	...
            	Log.d("test1", "ACTION_MOVE mInitialMotionY="+mInitialMotionY);
                if (!mIsBeingDragged && mInitialMotionY > 0f) {
                    final float yDiff = y - mInitialMotionY;
                    final float xDiff = x - mInitialMotionX;
                    Log.d("hyytest", "mTouchSlop="+mTouchSlop);
                    Log.d("test1", "Diff="+yDiff);
                    
                    if (Math.abs(yDiff) > Math.abs(xDiff) && (((mrefreshposition == 2|| mrefreshposition == 4 || mrefreshposition == 6) && yDiff > mTouchSlop)||((mrefreshposition == 3||mrefreshposition == 5 || mrefreshposition == 6) && yDiff < -mTouchSlop))) {
                        mIsBeingDragged = true;
                        Log.d("hyytest", "ACTION_MOVE 111111111111111yDiff="+yDiff);
                        if((mrefreshposition == 2 || mrefreshposition == 4 ||mrefreshposition == 6)&& yDiff > mTouchSlop){
                        	mrefreshmode = 1;//ÏÂÀ­Ë¢ÐÂ
                        	mHeaderTransformer.setheadertextview("ÏÂÀ­Ë¢ÐÂ");
                        	onPullStarted(y);
                        }else if((mrefreshposition == 3|| mrefreshposition ==5 ||mrefreshposition == 6)&& yDiff < -mTouchSlop){
                        	mrefreshmode = 2;
                        	//mHeaderTransformer.setheadertextview("ï¿½ï¿½ï¿½ï¿½Ë¢ï¿½ï¿½");
                        	//onPullStarted(y);
                        	mPullBeginY = y;
                        	onPulldown(mViewBeingDragged, y);
                        	mIsBeingDragged = false;
                        }
                        
                    } 
                    else if (yDiff < -mTouchSlop) {
                    	Log.d("hyytest", "ACTION_MOVE 222222222222222yDiff"+yDiff);
                        resetTouch(); //mIsBeingDragged = false
                    }
                }
                break;
            }
        ...
        if (DEBUG) Log.d("hyytest", "onInterceptTouchEvent. Returning " + mIsBeingDragged);
        Log.d("hyytest", "onInterceptTouchEvent. Returning " + mIsBeingDragged);
        ...
        if (view.isShown() && mRefreshableViews.containsKey(view)) {
            ...
            if (mRect.contains(rawX, rawY)) {
                ...
                if (delegate != null) {
                	...
                	mrefreshposition = delegate.isReadyForPullhyy(view, rawX - mRect.left, rawY - mRect.top);
                	
                     if(mrefreshposition==2||mrefreshposition==3||mrefreshposition==4||mrefreshposition==5||mrefreshposition==6){
                    	 Log.d("hyytest", "3**************3mrefreshposition="+mrefreshposition);
                    	 return true;
                    }
                }
            }
    ...
    final boolean onTouchEvent(MotionEvent event) {
        if (DEBUG) {
            Log.d("hyytest", "onTouchEvent: " + event.toString());
        }
        Log.d("hyytest", "onTouchEvent: ");
        //Log.d("hyytest", "onTouchEvent: " + event.toString());
    ...
    public void onPulldown(View view, float y) {
        if (DEBUG) {
            Log.d(LOG_TAG, "onPull");
        }
        Log.d("test5", "onPull");
    ...
    private void startRefresh(View view, boolean fromTouch) {
        ...
        if(mrefreshmode==1){
            ...
            showHeaderView();
        	 ...
        	 //mHeaderTransformer.onRefreshStarted();
        	//Toast.makeText(mActivity, "ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ë¢ï¿½ï¿½", 1).show();
        	//mHeaderTransformer.onRefreshStarted();
        	AbsListView absListView = (AbsListView) view;
        	Log.d("hyytest", ""+absListView.getCount());
        }
