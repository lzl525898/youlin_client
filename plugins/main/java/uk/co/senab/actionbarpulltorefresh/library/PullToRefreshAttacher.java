...
public class PullToRefreshAttacher {
    ...
    protected PullToRefreshAttacher(Activity activity, Options options) {
        ...
        if (options == null) {
            Loger.i(LOG_TAG, "Given null options so using default options.");
            options = new Options();
        }
    ...
    void addRefreshableView(View view, ViewDelegate viewDelegate) {
        ...
        if (view == null) {
            Loger.i(LOG_TAG, "Refreshable View is null.");
            return;
        }
    ...
    final boolean onInterceptTouchEvent(MotionEvent event) {
        if (DEBUG) {
            Loger.d(LOG_TAG, "onInterceptTouchEvent: " + event.toString());
        }
        ...
        if (DEBUG) Loger.d(LOG_TAG, "onInterceptTouchEvent. Returning " + mIsBeingDragged);
    ...
    final boolean isViewBeingDragged(View view, MotionEvent event) {
        if (view.isShown() && mRefreshableViews.containsKey(view)) {
            ...
            if (DEBUG) Loger.d(LOG_TAG, "isViewBeingDragged. View Rect: " + mRect.toString());
    ...
    final boolean onTouchEvent(MotionEvent event) {
        if (DEBUG) {
            Loger.d(LOG_TAG, "onTouchEvent: " + event.toString());
        }
    ...
    private void setRefreshingInt(View view, boolean refreshing, boolean fromTouch) {
        ...
        if (DEBUG) Loger.d(LOG_TAG, "setRefreshingInt: " + refreshing);
    ...
    private boolean isDestroyed() {
        if (mIsDestroyed) {
            Loger.i(LOG_TAG, "PullToRefreshAttacher is destroyed.");
        }
        return mIsDestroyed;
    }
