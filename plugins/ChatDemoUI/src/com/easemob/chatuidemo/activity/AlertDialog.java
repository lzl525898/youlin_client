...
public class AlertDialog extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.alertdialogforclear);
    ...
    public void ok(View view){
        Loger.d("test4", "AlertDialog gettype="+type);
        Intent intent = new Intent();
        intent.putExtra("position", position);
        intent.putExtra("edittext", editText.getText().toString());
        intent.putExtra("type", type);
        setResult(RESULT_OK,intent );/*.putExtra("voicePath", voicePath)*/
        if(position != -1)
...
//        intent.putExtra("edittext", editText.getText().toString());
//        intent.putExtra("type", type);
        setResult(RESULT_CANCELED,intent );/*.putExtra("voicePath", voicePath)*/
        finish();
    }
