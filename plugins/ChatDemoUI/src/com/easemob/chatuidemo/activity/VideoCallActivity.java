...
public class VideoCallActivity extends CallActivity implements OnClickListener {
    ...
    private int i=0;
    private Timer timer;
    public static Context videoContex;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 设置通话人
        NeighborDaoDBSearch search = new NeighborDaoDBSearch(this);
        if(search.findPointFamilyandUserObject(Long.parseLong(username)).size()<=0){
            nickTextView.setText(username);
        }else{
            nickTextView.setText(search.findPointFamilyandUserObject(Long.parseLong(username)).get(0));
        }
        // 显示本地图像的surfaceview
...
//                                        callStateTextView.setText(s6);
                            ...
                            public void run() {
                                ...
                                runOnUiThread(new Runnable() {
                                    public void run() {
                                        ...
                                        Loger.i("youlin","333333333");
                                        Toast.makeText(VideoCallActivity.this, "网络不稳定", Toast.LENGTH_SHORT).show();
                                    }
                                });
                            }
                        },30000);
    ...
    public void onClick(View v) {
        ...
        if(v.getId()==R.id.btn_answer_call){ // 接听电话
        ...
        if (v.getId()==R.id.btn_hangup_call){ // 挂断电话
            hangupBtn.setEnabled(false);
            if (soundPool != null)
                soundPool.stop(streamID);
            chronometer.stop();
        ...
        if (v.getId()==R.id.iv_handsfree){ // 免提开关
            if (isHandsfreeState) {
                // 关闭免提
