...
public class HXNotifier {
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        NeighborDaoDBSearch search = new NeighborDaoDBSearch(this.appContext);
        //String username = message.getFrom();
        String username = search.findPointFamilyandUserObject(Long.parseLong(message.getUserName())).get(0);
        try {
            ...
            if (notificationInfoProvider != null) {
                ...
                if (customNotifyText != null){
                    // 设置自定义的状态栏提示内容
//                    String[] s= customNotifyText.split(":");
//                    notifyText = search.findPointFamilyandUserObject(Long.parseLong(message.getUserName())).get(0)+":"+s[1];
                }
            ...
            NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(appContext)
                                                                        .setSmallIcon(R.drawable.pic_youlin_notify)
                                                                        .setLargeIcon(BitmapFactory.decodeResource(appContext.getResources(), appContext.getApplicationInfo().icon))
            ...
            if (notificationInfoProvider != null) {
                ...
                msgIntent.putExtra("userId",message.getUserName());
                msgIntent.putExtra("chatType", CHATTYPE_SINGLE);
                msgIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);  
                msgIntent.putExtra("selfurl", search.findSelfImg(Long.parseLong(DemoApplication.getInstance().getUserName())).get(0));
                msgIntent.putExtra("usernick", search.findPointFamilyandUserObject(Long.parseLong(message.getUserName())).get(0));
                msgIntent.putExtra("neighborurl",search.findPointFamilyandUserimg(Long.parseLong(message.getUserName())).get(0));
            }
            
            Intent intent2 = new Intent();  
            intent2.setClassName("com.nfs.youlin", "com.nfs.youlin.activity.InitTransparentActivity");
            intent2.putExtra("whereflag", "notification");
            intent2.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);//
            Intent[] intents = new Intent[2];  
            intents[0] = intent2;
            intents[1] = msgIntent;
//            PendingIntent pendingIntent = PendingIntent.getActivity(appContext, notifyID, intent2,PendingIntent.FLAG_UPDATE_CURRENT);
            PendingIntent pendingIntent = PendingIntent.getActivities(appContext, notifyID, intents,PendingIntent.FLAG_UPDATE_CURRENT);
            if(numIncrease){
