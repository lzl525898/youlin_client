package com.nfs.youlin.activity.titlebar.startactivity.zoom;
...
public class PhotoViewAttacher implements IPhotoView, View.OnTouchListener,
	...
	View view;
	Handler mHandler;  
	public PhotoViewAttacher(final Context context,final ImageView imageView) {
			...
			mGestureDetector = new GestureDetector(imageView.getContext(),
					new GestureDetector.SimpleOnGestureListener() {
						...
						public void onLongPress(MotionEvent e) {
							...
							if(view==null){
								view=LayoutInflater.from(mContext).inflate(R.layout.save_pic, null);
							}
							LinearLayout layout=(LinearLayout) view.findViewById(R.id.save_pic_layout);
					      ...
					      /******************以上代码用来循环检测activity是否初始化完毕*************/ 
							...
							layout.setOnClickListener(new OnClickListener() {
								...
								public void onClick(View v) {
									...
									try {
										dirPath=Environment.getExternalStorageDirectory()+"/youlin";
										File fileDir=new File(dirPath);
										if(!fileDir.exists()){
											fileDir.mkdir();
										}
										filePath=System.currentTimeMillis()+".jpg";
										file = new FileOutputStream(dirPath+"/"+filePath);
                                        ((BitmapDrawable)imageView.getDrawable()).getBitmap().compress(CompressFormat.JPEG, 100, file);
//										int tag = 0;
//										try {
...
//										}
//										if(tag==1){
//											((BitmapDrawable)((PhotoView)CircleDetailGalleryPictureActivity.galleryHeadImg).getDrawable()).getBitmap().compress(Bitmap.CompressFormat.JPEG, 100, file);
//										}else{
//											((BitmapDrawable)((PhotoView)GalleryPictureActivity.listViews.get(GalleryPictureActivity.pager.getCurrentItem())).getDrawable()).getBitmap().compress(Bitmap.CompressFormat.JPEG, 100, file);
//										}
									} catch (FileNotFoundException e1) {
										// TODO Auto-generated catch block
										e1.printStackTrace();
									}
									Toast.makeText(context, "保存路径为:"+dirPath+"/"+filePath, 1).show();
									popWindow.dismiss();
								}
							});
						}
	...
	public final void onDrag(float dx, float dy) {
		...
		if (DEBUG) {
			Loger.d(LOG_TAG, String.format("onDrag: dx: %.2f. dy: %.2f", dx, dy));
		}
	...
	public final void onFling(float startX, float startY, float velocityX,
			float velocityY) {
		if (DEBUG) {
			Loger.d(LOG_TAG, "onFling. sX: " + startX + " sY: " + startY
			...
			case MotionEvent.ACTION_DOWN:
				...
				new Timer().schedule(new TimerTask() {
					...
					public void run() {
						// TODO Auto-generated method stub
						timeBool=false;
						//Loger.i("TEST", "1111111111111111111111---->"+time1);
						if(time1==1 && time2==0){
							//Loger.i("TEST", "22222222222222222222222");
...
//				 Loger.i("TEST", "333333333333333333---->"+time1);
//				 //time1=0;
			case MotionEvent.ACTION_UP:
				...
				if (getScale() < mMinScale) {
					...
					//Loger.i("TEST", "55555555555555555555");
				}
	...
	private class FlingRunnable implements Runnable {
		...
		public void fling(int viewWidth, int viewHeight, int velocityX,
			...
			if (DEBUG) {
				Loger.d(LOG_TAG, "fling. StartX:" + startX + " StartY:" + startY
						+ " MaxX:" + maxX + " MaxY:" + maxY);
			}
		...
		public void run() {
			...
			if (null != imageView && mScroller.computeScrollOffset()) {
				...
				if (DEBUG) {
					Loger.d(LOG_TAG, "fling run(). CurrentX:" + mCurrentX
