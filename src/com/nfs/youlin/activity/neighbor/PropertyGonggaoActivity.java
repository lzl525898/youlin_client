...
public class PropertyGonggaoActivity extends FragmentActivity implements OnRefreshListener<ListView>,StatusChangeListener{
...
//	private final static int FIRST_NOINFO_SUCCESS_CODE = 2009;
	...
	int updatePosition;
	LinearLayout gonggaoInitLayout;
	...
	JPushInterface.onResume(PropertyGonggaoActivity.this);
...
//		this.requestWindowFeature(Window.FEATURE_CUSTOM_TITLE);
		...
		this.setContentView(R.layout.activity_property_gonggao);
//		this.getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE, R.layout.titlebar_search_item);
		...
		statusutils.statuschangelistener("GONGGAO",this);
		
		forumtopicLists.clear();
		nonemsgtext=(TextView)findViewById(R.id.gonggao_noinfo_tv1);
		gonggaoInitLayout=(LinearLayout)findViewById(R.id.gonggao_init);
		maddapter = new FriendCircleAdapter(forumtopicLists, PropertyGonggaoActivity.this,1,6);
			forumtopicLists.clear();
			...
			client.post(IHttpRequestUtils.URL+IHttpRequestUtils.YOULIN,
				...
				public void onSuccess(int statusCode, Header[] headers,
					...
					try {
						...
						if("no".equals(Flag)){
							cRequesttype = true;
							gonggaoInitLayout.setVisibility(View.VISIBLE);
				...
				public void onFailure(int statusCode, Header[] headers,
					...
					// TODO Auto-generated method stub
					new ErrorServer(PropertyGonggaoActivity.this, responseString.toString());
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					while (PropertyGonggaoActivity.this.getSupportFragmentManager().findFragmentByTag(
							"myfragment") == null) {
					}
					while(!cRequesttype ){
					...
					PropertyGonggaoActivity.this.runOnUiThread(new Runnable() {
						@Override
						public void run() {
							// TODO Auto-generated method stub
							...
							final PullToRefreshListFragment finalfrag9 = (PullToRefreshListFragment) PropertyGonggaoActivity.this.getSupportFragmentManager()
							...
							try {
								...
								mPullRefreshListView.setOnRefreshListener(PropertyGonggaoActivity.this);
...
//	}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		if(4 == type||5 == type||6 == type){//表示当前用户为物业管理员 //表示当前用户为物业管理员+申请管理员//表示当前用户为物业管理员+管理员
			getMenuInflater().inflate(R.menu.property_gonggao, menu);
			menu.getItem(0).setActionView(R.layout.property_advice_plus_layout);
			LinearLayout gonggaoAddLayout=(LinearLayout) menu.findItem(R.id.gonggao_add).getActionView();
			ImageView gonggaoAddImg=(ImageView) gonggaoAddLayout.findViewById(R.id.advice_add);
			gonggaoAddImg.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View v) {
					...
					if(addrDetails!=null){
						AllFamilyDaoDBImpl curfamilyDaoDBImpl = new AllFamilyDaoDBImpl(PropertyGonggaoActivity.this);
						...
						if(currentFamily.getEntity_type()==1 && currentFamily.getPrimary_flag()==1){
							...
							if(4 == type||5 == type||6 == type){
								startActivity(new Intent(PropertyGonggaoActivity.this,PropertyGonggaoAdd.class).addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT));
							}
						}else{
							showAddDialog();
						}
...
//				Loger.i("TEST", "mediaJson->start");
				...
				Loger.i("LYM", "12313---->"+topic_time+" "+App.CurrentSysTime);
...
//					Loger.i("TEST", "(E)getTopicComm->"+e.getMessage());
		...
		Loger.d("test5", "gonggao forumtopicLists size="+forumtopicLists.size()+""+((ForumTopic)forumtopicLists.get(0)).getTopic_title());
	...
	public void pullUpFromSrv(String search){
		...
		if(forumtopicLists.size()>0){
			...
			params.put("category_type",App.GONGGAO_TYPE);
			params.put("tag","getnotice");
...
//			reference.put("count", 6);	
			...
			client.post(IHttpRequestUtils.URL+IHttpRequestUtils.YOULIN,
				...
				public void onSuccess(int statusCode, Header[] headers,
						JSONArray response) {
					// TODO Auto-generated method stub 18945051215
					gonggaoInitLayout.setVisibility(View.GONE);
					getTopicDetailsInfos(response);
					Flag = "ok";
					cRequesttype = true;
//					maddapter.notifyDataSetChanged();
...
//									actualListView.setDivider(PropertyGonggaoActivity.this.getResources().getDrawable(R.drawable.fengexian));
	...
	protected void onPause() {
		// TODO Auto-generated method stub
		super.onPause();
		JPushInterface.onPause(PropertyGonggaoActivity.this);
	...
	private void gethttpdata(long topicId,int request_index){
		...
		reference.put("tag","getnotice");
		reference.put("apitype",IHttpRequestUtils.APITYPE[4]);
		Loger.i("youlin", "33333333333--->"+App.sFamilyCommunityId+" "+App.sUserLoginId+" "+topicId);
		AsyncHttpClient httpClient = new AsyncHttpClient();
		httpClient.post(IHttpRequestUtils.URL+ IHttpRequestUtils.YOULIN,reference,
						new JsonHttpResponseHandler() {
							@Override
							public void onSuccess(int statusCode,Header[] headers,JSONArray response) {
								Loger.i("TEST","5555555--->"+response.toString());
